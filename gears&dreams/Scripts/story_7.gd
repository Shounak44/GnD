...
@onready var anim_player: AnimationPlayer = $CanvasLayer3/AnimationPlayer

# Initial fade animation that plays on load
@export var fade_anim_name: String = "fade6"

# Dialogue sequence — panel names must match the Panel nodes under CanvasLayer
var dialogue_sequence := [
	{"speaker":"NaniPanel", "text":"And that's the story of Kalpataru, the wishing tree."},
	{"speaker":"MinaPanel", "text":"Hmm..."},
	{"speaker":"NaniPanel", "text":"What happened Mina? Didn't you like the story?"},
	{"speaker":"MinaPanel", "text":"No, I liked it... I am sad about that farmer."},
	{"speaker":"NaniPanel", "text":"It's okay Mina, pray for him."},
	{"speaker":"MinaPanel", "text":"I hope like the Kalpataru story, my dream will come true too."}
]
...
func _ready() -> void:
	...
	var panels = ["MinaPanel","NaniPanel"]
	for p in panels:
	...
	if event.is_action_pressed("ui_accept"):
		if is_typing:
			...
			if current_index < dialogue_sequence.size():
				...
				# sequence finished — go to next scene (change path as needed)
